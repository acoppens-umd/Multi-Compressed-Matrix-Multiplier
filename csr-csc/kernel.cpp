...
{
#pragma HLS cache port=a_in_port lines=16384 depth=64
#pragma HLS cache port=b_in_port lines=16384 depth=64
...
#pragma HLS DATAFLOW
    ...
    task_distributor(a_num_rows, b_num_cols,
                     a_meta_in_port, b_meta_in_port,
                     task_a_position_streams, task_b_position_streams);

    for (int pe = 0; pe < NUM_PES; pe++) {
        #pragma HLS UNROLL
        processing_element(task_a_position_streams[pe], task_b_position_streams[pe],
    ...
    for (int pe = 0; pe < NUM_PES; pe++) {
        #pragma HLS UNROLL
        processing_element_multiplier(b_num_cols, 
                                      a_data_streams[pe], b_data_streams[pe],
    ...
    write_back_network(gate_to_network_initiation_stream, gate_to_network_streams, 
