...
inline void swap_and_merge(DenseVal_t &in_val1, DenseVal_t &in_val2, 
    ...
    //same dest
    if (in_val1.dest_i == in_val2.dest_i && in_val1.dest_k == in_val2.dest_k) {
        out_val1.val = in_val1.val + in_val2.val;
        out_val1.dest_i = in_val1.dest_i;
        out_val1.dest_k = in_val1.dest_k;
        out_val1.valid = in_val1.valid || in_val2.valid;

        out_val2.val = 0.0f;
        out_val2.dest_i = in_val2.dest_i;
        out_val2.dest_k = in_val2.dest_k;
        out_val2.valid = false;

    //in_val1 first
    } else if (in_val1.dest_i < in_val2.dest_i ||
               (in_val1.dest_i == in_val2.dest_i &&
               in_val1.dest_k < in_val2.dest_k)) {
    ...
    //in_val2 first
        ...
        out_val1.dest_i = in_val2.dest_i;
        out_val1.dest_k = in_val2.dest_k;
        out_val1.valid = in_val2.valid;

        out_val2.val = in_val1.val;
        out_val2.dest_i = in_val1.dest_i;
        out_val2.dest_k = in_val1.dest_k;
...
inline void write_back(const int a_num_rows, const int b_num_cols, 
                       DenseVal_t (&out_array)[NUM_PES], ap_uint<32> *out_data,
                       int &atomic_write_counter) {
    write_back_loop:
    for (int wire = 0; wire < NUM_PES; wire++) {
        if (out_array[wire].valid) {
            ap_uint<32> index = out_array[wire].dest_k + out_array[wire].dest_i * b_num_cols;
            bit_float prev;
            prev.as_uint32 = out_data[index];
            out_data[index] = prev.as_float32 + out_array[wire].val;
